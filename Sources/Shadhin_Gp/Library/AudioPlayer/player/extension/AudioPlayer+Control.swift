...
extension AudioPlayer {
    ...
    public func previous(isUserForced: Bool = false) {
        if let previousItem = queue?.previousItem(isUserForced: isUserForced) {
    ...
    public func next(isUserForced: Bool = false) {
        if let queue = queue, let nextItem = queue.nextItem(isUserForced: isUserForced) {
    ...
    public func indexToPlay(index: Int, from: Int){
        let isForward = index > from
        let hopNumber = abs(index - from)
        for _ in 1...hopNumber{
            if isForward{
                next(isUserForced: true)
            }else{
                previous(isUserForced: true)
            }
        }
    }
    ...
    public func stop() {
        ...
        if let _ = queue, clearQueueWhenStopped {
            queue = nil
        }
    ...
    public func remoteControlReceived(with event: UIEvent) {
        ...
        case .remoteControlNextTrack:
            guard ShadhinPlayerSkipLimit.instance.isActionAllowed() else {
                if let vc = ShadhinCore.instance.getTopViewController(){
                    var style = ToastManager.shared.style
                    style.messageAlignment = .center
                    vc.view.makeToast("Free skip limit reached, Please subscribe or try after some time", style: style)
