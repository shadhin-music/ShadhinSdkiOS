...
extension AudioPlayer {
    ...
    public var currentItemIndexInQueue: Int? {
        return currentItem.flatMap { queue?.queue.firstIndex(of: $0) }
    }
    ...
    func playItem(at : Int){
        guard let queue = queue, queue.items.count > at, at > -1 else {return}
        currentItem = queue.items[at]
        if !queue.mode.contains(.repeat){
            queue.nextPosition = at + 1
        }else{
            queue.nextPosition = at
        }
