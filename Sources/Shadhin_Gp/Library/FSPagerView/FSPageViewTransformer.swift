...
@objc
 enum FSPagerViewTransformerType: Int {
    case crossFading
    case zoomOut
    case depth
    case overlap
 ...
 class FSPagerViewTransformer: NSObject {
    
     weak var pagerView: FSPagerView?
     var type: FSPagerViewTransformerType
    
    @objc  var minimumScale: CGFloat = 0.65
    @objc  var minimumAlpha: CGFloat = 0.6
    
    @objc
     init(type: FSPagerViewTransformerType) {
        self.type = type
        switch type {
        case .zoomOut:
    ...
    // Apply transform to attributes - zIndex: Int, frame: CGRect, alpha: CGFloat, transform: CGAffineTransform or transform3D: CATransform3D.
     func applyTransform(to attributes: FSPagerViewLayoutAttributes) {
        ...
        case .depth:
            ...
            attributes.transform = transform
            attributes.zIndex = zIndex
        case .overlap,.linear,.linearDeen:
    ...
    // An interitem spacing proposed by transformer class. This will override the default interitemSpacing provided by the pager view.
     func proposedInteritemSpacing() -> CGFloat {
        ...
        case .linearDeen:
            ...
            return pagerView.itemSize.width * -0.3 * 0.1
        case .coverFlow:
            guard scrollDirection == .horizontal else {
                return 0
            }
