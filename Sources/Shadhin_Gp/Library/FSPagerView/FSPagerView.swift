...
@objc
 protocol FSPagerViewDataSource: NSObjectProtocol {
...
@objc
 protocol FSPagerViewDelegate: NSObjectProtocol {
...
@IBDesignable
 class FSPagerView: UIView,UICollectionViewDataSource,UICollectionViewDelegate {
    
    // MARK: - Public properties

    /// The object that acts as the data source of the pager view.
    @IBOutlet  weak var dataSource: FSPagerViewDataSource?
    
    /// The object that acts as the delegate of the pager view.
    @IBOutlet  weak var delegate: FSPagerViewDelegate?
    ...
    @objc
     var transformer: FSPagerViewTransformer? {
        didSet {
    ...
    @objc(dequeueReusableCellWithReuseIdentifier:atIndex:)
     func dequeueReusableCell(withReuseIdentifier identifier: String, at index: Int) -> FSPagerViewCell {
    ...
    @objc(indexForCell:)
     func index(for cell: FSPagerViewCell) -> Int {
    ...
    @objc(cellForItemAtIndex:)
     func cellForItem(at index: Int) -> FSPagerViewCell? {
        let indexPath = self.nearbyIndexPath(for: index)
