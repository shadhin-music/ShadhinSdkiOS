...
internal class ReconnectableConnection: Connection {
    ...
    private func restartConnection(error: Error?) {
        ...
        if currentState == .starting || currentState == .reconnecting {
            ...
            logger.log(logLevel: .debug, message: "nextAttemptInterval: \(nextAttemptInterval), SRRetryContext: \(retryContext)")
    ...
    private func updateAndCreateRetryContext(error: Error?) -> SRRetryContext {
        var attemptsCount = -1
        var startTime = Date()
        connectionQueue.sync {
        ...
        return SRRetryContext(failedAttemptsCount: attemptsCount, reconnectStartTime: startTime, error: error)
    }
