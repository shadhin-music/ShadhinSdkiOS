 ...
 extension EKAttributes {
    ...
    struct UserInteraction {
        ...
        /** Additional actions that can be customized by the user */
         var customTapActions: [Action]
        
         init(defaultAction: Default = .absorbTouches, customTapActions: [Action] = []) {
            self.defaultAction = defaultAction
            self.customTapActions = customTapActions
        }
        
        /** Dismiss action */
         static var dismiss: UserInteraction {
            return UserInteraction(defaultAction: .dismissEntry)
        }
        
        /** Forward action */
         static var forward: UserInteraction {
            return UserInteraction(defaultAction: .forward)
        }
        
        /** Absorb touches action */
         static var absorbTouches: UserInteraction {
            return UserInteraction(defaultAction: .absorbTouches)
        }
        
        /** Delay exit action */
         static func delayExit(by delay: TimeInterval) -> UserInteraction {
