 ...
 */
 final class SwiftEntryKit {
     ...
     */
     class var isCurrentlyDisplaying: Bool {
     ...
     */
     class func isCurrentlyDisplaying(entryNamed name: String? = nil) -> Bool {
    ...
    /**
     ...
     - parameter name: The name of the entry. Its default value is *nil*.
     */
     class func queueContains(entryNamed name: String? = nil) -> Bool {
     ...
     */
     class func display(entry view: UIView, using attributes: EKAttributes, presentInsideKeyWindow: Bool = false, rollbackWindow: RollbackWindow = .main) {
    ...
    /**
     ...
     - parameter rollbackWindow: After the entry has been dismissed, SwiftEntryKit rolls back to the given window. By default it is *.main* - which is the app main window
     */
     class func display(entry viewController: UIViewController, using attributes: EKAttributes, presentInsideKeyWindow: Bool = false, rollbackWindow: RollbackWindow = .main) {
        DispatchQueue.main.async {
     ...
     */
     class func transform(to view: UIView) {
        DispatchQueue.main.async {
     ...
     */
     class func dismiss(_ descriptor: EntryDismissalDescriptor = .displayed, with completion: DismissCompletionHandler? = nil) {
        DispatchQueue.main.async {
