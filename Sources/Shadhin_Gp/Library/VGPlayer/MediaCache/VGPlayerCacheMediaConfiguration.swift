...
import Foundation
 ...
 class VGPlayerCacheMediaConfiguration:NSObject, NSCoding, NSCopying {
    
     fileprivate(set) var filePath: String?
     fileprivate(set) var cacheSegments = [NSValue]()
    ...
    fileprivate var downloadInfo = [Any]()
     ...
     fileprivate(set) var downloadSpeed: Double? { // kb/s
        didSet {
            var bytes: UInt64 = 0
            var time = 0.0
    ...
    // NSCoding & NSCoying
     ...
     static func filePath(for filePath: String) -> String {
        let nsString = filePath as NSString
        return nsString.appendingPathExtension("conf")!
    }
    
     static func configuration(filePath: String) -> VGPlayerCacheMediaConfiguration {
        let path = self.filePath(for: filePath)
        
        do{
            let archive = try NSKeyedArchiver.archivedData(withRootObject: path, requiringSecureCoding: false)
            let config = try NSKeyedUnarchiver.unarchivedObject(ofClass: VGPlayerCacheMediaConfiguration.self, from: archive)
...
extension VGPlayerCacheMediaConfiguration {
     ...
     func addCache(_ segment: NSRange) {
     ...
     func add(_ downloadedBytes: UInt64, time: TimeInterval) {
