...
import UIKit
 ...
 class VGPlayerCacheMediaWorker: NSObject {
     fileprivate(set) var cacheConfiguration: VGPlayerCacheMediaConfiguration?
     fileprivate(set) var setupError: Error?
    ...
    deinit {
     ...
     func cache(_ data: Data, forRange range: NSRange, closure: (Bool) -> Void) {
        writeFileQueue.sync {
     ...
     func cache(forRange range: NSRange) -> Data? {
        readFileHandle?.seek(toFileOffset: UInt64(range.location))
        let data = readFileHandle?.readData(ofLength: range.length)
        return data
    }
    
     func cachedDataActions(forRange range:NSRange) -> Array<VGPlayerCacheAction> {
     ...
     func set(cacheMedia: VGPlayerCacheMedia) -> Bool {
