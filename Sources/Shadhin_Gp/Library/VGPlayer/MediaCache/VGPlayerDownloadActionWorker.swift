...
import Foundation
 ...
 protocol VGPlayerDownloadActionWorkerDelegate: NSObjectProtocol {
 ...
 class VGPlayerDownloadActionWorker: NSObject {
    
     fileprivate(set) var actions = [VGPlayerCacheAction]()
     fileprivate(set) var url: URL
     fileprivate(set) var cacheMediaWorker: VGPlayerCacheMediaWorker
    
     fileprivate(set) var session: URLSession?
     fileprivate(set) var task: URLSessionDataTask?
     fileprivate(set) var downloadURLSessionManager: VGPlayerDownloadURLSessionManager?
     fileprivate(set) var startOffset: Int = 0
    
     weak var delegate: VGPlayerDownloadActionWorkerDelegate?
    
    fileprivate var isCancelled: Bool = false
    fileprivate var notifyTime = 0.0
    
    
     init(actions: [VGPlayerCacheAction], url: URL, cacheMediaWorker: VGPlayerCacheMediaWorker) {
