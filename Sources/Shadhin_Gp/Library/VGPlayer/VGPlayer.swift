...
/// - proceed: continue
 enum VGPlayerBackgroundMode: Int {
    case suspend
    case autoPlayAndPaused
    case proceed
}

 protocol VGPlayerDelegate: AnyObject {
    // play state
...
// MARK: - delegate methods optional
 ...
 class VGPlayer: NSObject {
    
     var state: VGPlayerState = .none {
        didSet {
            if state != oldValue {
     ...
     var bufferState : VGPlayerBufferstate = .none {
        didSet {
     ...
     var displayView : VGPlayerView
    
     var gravityMode : VGVideoGravityMode = .resizeAspect
     var backgroundMode : VGPlayerBackgroundMode = .autoPlayAndPaused
     var bufferInterval : TimeInterval = 2.0
     weak var delegate : VGPlayerDelegate?
    
     fileprivate(set) var mediaFormat : VGPlayerMediaFormat
    ...
    private var timeObserver: Any?
     ...
     fileprivate(set) var error : VGPlayerError
    ...
    //MARK:- life cycle
     init(URL: URL?, playerView: VGPlayerView?) {
     ...
     convenience init(playerView: VGPlayerView) {
...
//        }
    ...
    internal func removePlayerObservers() {
     ...
     var playBackStartTime : Date?{
         get {
             if let accessLog = player?.currentItem?.accessLog(), let event = accessLog.events.first{
...
extension VGPlayer {
     ...
     func seekTime(_ time: TimeInterval, completion: ((Bool) -> Swift.Void)?) {
