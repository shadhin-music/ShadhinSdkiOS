 ...
 */
 struct subtitles: CustomStringConvertible {
     var index : Int
 ...
 class VGSubtitles {
     ...
     fileprivate(set) var subtitlesFormat : subtitlesFormat = .unknown
    // 存放字幕的字典  时间戳key 字幕value
     fileprivate(set) var subtitlesGroups : [subtitles] = []
    
     init(filePath: URL, encoding: String.Encoding = String.Encoding.utf8) {
     ...
     func search(for time: TimeInterval) -> subtitles? {
